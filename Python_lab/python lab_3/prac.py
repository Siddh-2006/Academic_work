...
for i in range(len(str) - len(substring) , 0 ,-1):
    if str[i:i+len(substring)] == substring:
        print(i)
        break
# print(next(i for i in range(len(str) - len(substring) , 0 ,-1) if str[i:i+len(substring)] == substring))
...
# print( [part[::-1] for part in s[::-1].split(',', 2)][::-1] )
...
# print(len(s) - s[::-1].find(sub[::-1]) - len(sub))
